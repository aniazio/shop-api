...
create table order_details (total float(53) not null, created_at timestamp(6) not null, id bigint not null, user_id bigint not null, status varchar(255) not null check (status in ('CART','ORDERED','CANCELED')), primary key (id));
create table order_item (price float(53) not null, quantity integer not null, order_id bigint not null, product_id bigint not null, primary key (order_id, product_id));
create table product (available integer not null, price float(53) not null, id bigint not null, title varchar(255) not null, primary key (id));
